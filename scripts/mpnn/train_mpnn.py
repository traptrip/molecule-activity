...
def train():
    ...
    x_train = graphs_from_smiles(
        df.loc[train_index].Smiles, atom_featurizer, bond_featurizer
    )
    y_train = df.loc[train_index].Active

    # Valid set
    x_valid = graphs_from_smiles(
        df.loc[valid_index].Smiles, atom_featurizer, bond_featurizer
    )
    ...
    mpnn.save_weights("./models/mpnn_10k.h5")
